...
class ClassificationHead_2(nn.Module):
...
class ClassificationHead_1(nn.Module):
    """Head for sentence-level classification tasks."""
...
class myModelForMultipleChoice(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.electra = ElectraModel(config)
        self.classifier_2 = ClassificationHead_2(
            config)  # nn.Linear(config.hidden_size, 2)
        self.classifier_1 = ClassificationHead_1(
            config)  # nn.Linear(config.hidden_size, 1)
        self.init_weights()
    ...
    def forward(
        ...
        logits = self.classifier_2(sequence_output)
        ...
        if labels is not None:
            reshaped_labels = []
            for label in labels:
                for _ in range(0, label):
                    reshaped_labels.append(0)
                reshaped_labels.append(1)
                for _ in range(label+1, num_choices):
                    reshaped_labels.append(0)
            reshaped_labels = torch.tensor(
                reshaped_labels, dtype=torch.long).to(device)

            loss_fct = nn.CrossEntropyLoss()
            loss_1 = loss_fct(logits, reshaped_labels)

            logits = torch.tensor([logit[1] for logit in logits]).to(device)
            logits = logits.view(-1, n_class)

            ordlogits = np.argsort(logits.cpu().numpy())
            input_ids_index = []
            labels_ = []
            labels_cpu = labels.cpu().numpy()
            for i in range(logits.shape[0]):
                input_ids_index.append((
                    num_choices*i+ordlogits[i][num_choices-1]).item())
                if ordlogits[i][num_choices-1] == labels_cpu[i] or ordlogits[i][num_choices-2] == labels_cpu[i]:
                    input_ids_index.append((
                        num_choices*i+ordlogits[i][num_choices-2]).item())
                    if ordlogits[i][num_choices-1] == labels_cpu[i]:
                        labels_.append(0)
                    else:
                        labels_.append(1)
                else:
                    input_ids_index.append(
                        (num_choices*i+labels_cpu[i]).item())
                    labels_.append(1)
            sequence_output_=sequence_output[input_ids_index]
            labels_ = torch.tensor(labels_, dtype=torch.long).to(device)
            logits_ = self.classifier_1(sequence_output_)
            logits_ = logits_.view(-1, 2)
            loss_2 = loss_fct(logits_, labels_)
            ...
            for i in range(logits.shape[0]):
                input_ids_index.append(
                    num_choices*i+ordlogits[i][num_choices-1])
                input_ids_index.append(
                    num_choices*i+ordlogits[i][num_choices-2])
                    
            sequence_output_=sequence_output[input_ids_index]
            logits_ = self.classifier_1(sequence_output_)
            logits_ = logits_.view(-1, 2)
            final_logits = torch.tensor(-10 * np.ones(
                logits.cpu().numpy().shape, logits.cpu().numpy().dtype)).to(device)
            for i in range(logits.shape[0]):
                final_logits[i][ordlogits[i][num_choices-1]] = logits_[i][0]
                final_logits[i][ordlogits[i][num_choices-2]] = logits_[i][1]
...
class c3Processor(DataProcessor):
    def __init__(self, data_dir):
        ...
        for sid in range(3):
            data = []
            for subtask in ["d", "m"]:
                with open(self.data_dir + "/race-" + subtask + "-" + ["train.json", "dev.json", "test.json"][sid],
            ...
            for (i, d) in enumerate(data):
                ...
                for k in range(4):
                    ...
                    examples.append(InputExample(
                        guid=guid, text_a=text_a, text_b=text_b, label=label, text_c=text_c))
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        tokens_a = tokenizer.tokenize(example.text_a)  # context

        tokens_b = tokenizer.tokenize(example.text_b)  # 答案

        tokens_c = tokenizer.tokenize(example.text_c)  # 问题
...
def main():
    ...
    assert args.model_type=='large', 'only accept large model'
    # pretrained = 'superspray/electra_large_discriminator_squad2_custom_dataset'
    pretrained = 'google/electra-large-discriminator'
    ...
    type_vocab_size = 3
    temp_model = myModelForMultipleChoice.from_pretrained(pretrained)
    torch.save(temp_model.state_dict(), 'model_weights_save')
    state_dict = torch.load('model_weights_save')

    tte_weight_1 = state_dict["electra.embeddings.token_type_embeddings.weight"][1].clone(
    ).unsqueeze(0)
    state_dict["electra.embeddings.token_type_embeddings.weight"] = torch.cat(
        [state_dict["electra.embeddings.token_type_embeddings.weight"], tte_weight_1], dim=0)
    
    # temp_state_dict = torch.load('check_points/exc_race/electra-exc-2-16-large-drp0.1-wp1-loss2/model_best.pt')
    # state_dict={}
    ...
    #     if key[:8]=="electra.":
    #         temp_key=temp_key[8:]
    #     state_dict[temp_key]=temp_state_dict[key]

    model_config = ElectraConfig.from_pretrained(
        pretrained, type_vocab_size=type_vocab_size, attention_probs_dropout_prob=args.a_dropout_prob, hidden_dropout_prob=args.h_dropout_prob,
        summary_last_dropout=args.s_dropout_prob)

    model = myModelForMultipleChoice.from_pretrained(
        pretrained, config=model_config, state_dict=state_dict)
    ...
    if args.do_eval:
        ...
        feature_dir = os.path.join(
            args.data_dir, 'emask_dev_features{}.pkl'.format(args.max_seq_length))
        ...
        eval_dataloader = DataLoader(
            eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size, drop_last=False)

    if args.do_train:
        best_accuracy = 0

        feature_dir = os.path.join(
            args.data_dir, 'emask_train_features{}.pkl'.format(args.max_seq_length))
        if os.path.exists(feature_dir):
            ...
            with tqdm(total=int(steps_per_epoch), desc='Epoch %d' % (ie + 1)) as pbar:
                for step, batch in enumerate(train_dataloader):
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        nb_tr_steps += 1
                        pbar.set_postfix(
                            {'loss': '{0:1.5f}'.format(tr_loss / (nb_tr_steps + 1e-5))})
                        pbar.update(1)
    ...
    if args.do_eval:
        ...
        feature_dir = os.path.join(
            args.data_dir, 'emask_test_features{}.pkl'.format(args.max_seq_length))
        if os.path.exists(feature_dir):
        ...
        test_dataloader = DataLoader(
            test_data, sampler=test_sampler, batch_size=args.eval_batch_size, drop_last=False)
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(test_dataloader):
            ...
            with torch.no_grad():
                ...
                logits = outputs['logits']
                tmp_test_loss = loss_fct(logits, label_ids)
